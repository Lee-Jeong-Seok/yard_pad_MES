...
### 6.1 시뮬레이션 및 사전 검증 (개발 단계)
* **MSSQL 쿼리 및 트랜잭션 무결성 검증**: 검사/선별/이송/인계 시 데이터 중복 업데이트 방지 및 롤백 검증
* **갤럭시 탭 A9+ 디바이스 에뮬레이션**: 11인치 1920x1200 화면비율 및 세로/가로 회전, 화면 분할 터치 반응 검증
* **카메라 스캐너 성능 검증**: 1D Barcode(Code 128) 및 2D QR 코드에 대한 브라우저 인식률 및 스캔 속도 테스트
* **사진 압축 효율 검증**: 고해상도(10MB+) 원본 촬영 사진이 브라우저에서 200~400KB로 0.3초 내 자동 압축되어 서버로 전송되는지 검증

### 6.2 야드 현장 실물 PoC 검증 (배포 단계)
* 갤럭시 탭 A9+ 1대로 현장 Wi-Fi 환경에서 실제 자재 검사 및 사진 촬영 테스트
* 바코드 스캔 -> MSSQL 재고 위치 업데이트까지 실시간 반영 지연 시간(Latency < 1초) 확인
* 인수자 전자서명이 정상 저장되고 PC 관리자 화면에서 즉시 열람되는지 확인
